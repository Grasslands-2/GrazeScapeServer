...
public class ServerStartup {
    ...
    public ServerStartup(ApplicationLifecycle lifecycle, play.Environment env) {
        ...
        Model_CropYield model = new Model_CropYield();
        Boolean result = model.initialize();
        logger.debug(String.valueOf(result));
        try{
            long[][] modelResult;
            modelResult = model.run();
            logger.debug(String.valueOf(modelResult));
            logger.debug(String.valueOf(modelResult.length));
            for(long i=0; i< modelResult.length;i++){
                for(long j=0; j< modelResult[0].length;j++){
                    logger.debug(String.valueOf("HI"));
                }
            }
        }
        catch(Exception e){
...
//		Commented out by Matthew: File is missing
//         testPMML_rf();
...
//		String modelPath = ServerStartup.getApplicationRoot() + "/conf/testModels/contPast.pmml";
        ...
        try {
            ...
            logger.info("PMML Load: " + timer.stringSeconds(3));
            timer.restartTimer();

            evaluator = (Evaluator) lmeb.build();
            logger.info("PMML Evaluator build: " + timer.stringSeconds(3));
            timer.restartTimer();
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        evaluator.verify();
        logger.info("PMML self-check: " + timer.stringSeconds(3));
        timer.restartTimer();
        ...
        logger.info("Before raster evaluation: " + timer.stringSeconds(3));
        timer.restartTimer();
...
*/
        ...
        logger.info("After raster evaluation: " + timer.stringSeconds(3));
        timer.restartTimer();
    }
